...
def get_pop_acc_pres_abs_panaroo(data_path, out_path, acc_upper, acc_lower):
    try:
        ...
        # Now make gene names the indices
        ...
        # Get list of sample names in the subpopulation to analyze
        ...
        # Ensure the main script block is only executed when the script is run directly
        if __name__ == '__main__':
            ...
            # Append the missing representative sequences to the existing acc gene fasta file
        ...
        print(f"Error in get_pop_acc_pres_abs_panaroo: {e}")
...
def get_pop_core_pres_abs_panaroo(data_path, out_path, core_lower):
    try:
        ...
        def get_freq(dataframe):
            ...
            freq_dataframe = pd.DataFrame({'COG_name': cog_list, 'freq': freq_list})
            return freq_dataframe
        ...
        core_genes_notin_out = list(set(corecog_name_list) - set(core_genes_in_out))
        ...
        ### Add the missing representative sequences to the existing core gene fasta file
        ...
        print(f"Error in get_pop_core_pres_abs_panaroo: {e}")
